...
class ActivityLogFormatter
{
    ...
    public function eventsForSubject(Model $subject, ?string $tenantId): Collection
    {
        return Activity::query()
            ...
            ->orderBy('event')
            ->pluck('event')
            ->mapWithKeys(fn (string $event): array => [$event => $this->eventFilterLabel($event)]);
    }
    ...
    {
        return match ($event) {
            'created', 'activated' => 'bg-green-100 text-green-600',
            'updated' => 'bg-blue-100 text-blue-600',
            'deleted', 'suspended' => 'bg-red-100 text-red-600',
            'restored' => 'bg-purple-100 text-purple-600',
            'session_kill' => 'bg-amber-100 text-amber-600',
    ...
    {
        return match ($event) {
            'created', 'activated' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>',
            ...
            'suspended', 'session_kill' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>',
    ...
    private function eventFilterLabel(string $event): string
    {
        return match ($event) {
            'activated' => 'Activate',
            'suspended' => 'Suspend',
