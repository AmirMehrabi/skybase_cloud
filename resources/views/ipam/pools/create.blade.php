...
@section('content')
<div class="space-y-6" x-data="createPoolForm()" x-effect="calculatePreview(form.networkAddress, form.cidr)">
    <!-- Header -->
...
function createPoolForm() {
    return {
        form: {
            name: '{{ old('name') }}',
            siteId: '{{ old('site_id') }}',
            routerIds: @json(collect(old('router_ids', []))->map(fn ($routerId) => (string) $routerId)->values()->all()),
            allDevices: {{ old('all_devices', false) ? 'true' : 'false' }},
            type: '{{ old('type', 'mixed') }}',
