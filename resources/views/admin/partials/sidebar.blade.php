@php
    ...
    $canSettings = $can('admin.tenant.users.index') || $can('admin.tenant.user-groups.index') || $can('admin.tenant.roles.index') || $can('settings.index');
@endphp
...
<ul class="space-y-1">
    ...
    <!-- User Groups -->
    @if($can('admin.tenant.user-groups.index'))
    <li>
        <a href="{{ route('admin.tenant.user-groups.index') }}"
           class="flex items-center gap-3 rounded-lg border px-3 py-2 text-sm font-medium transition {{ str_starts_with($currentRoute, 'admin.tenant.user-groups') ? 'border-white/15 bg-white/[0.12] text-white shadow-sm' : 'border-transparent text-teal-50/85 hover:border-white/10 hover:bg-white/10 hover:text-white' }}">
