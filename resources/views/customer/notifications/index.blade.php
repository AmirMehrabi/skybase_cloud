...
<div class="mx-auto max-w-5xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Service, billing, support, and usage updates for your account.</p>
        </div>
    ...
    <form method="GET" action="{{ route('customer.notifications.index') }}" class="grid grid-cols-1 gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-3">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div class="flex flex-col gap-4 border-b border-gray-100 p-5 last:border-b-0 md:flex-row md:items-start md:justify-between {{ $notification->read_at ? 'bg-white' : 'bg-blue-50/40' }}">
                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600">{{ ucfirst($data['category'] ?? 'service') }}</span>
                    </div>
                ...
                <div class="flex shrink-0 flex-wrap items-center gap-2">
                    ...
                    @if(! $notification->read_at)
                        <form method="POST" action="{{ route('customer.notifications.read', ['notification' => $notification->id]) }}">
                            @csrf
                            @method('PATCH')
                    ...
                    @endif
                    <form method="POST" action="{{ route('customer.notifications.archive', ['notification' => $notification->id]) }}">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <div class="px-6 py-12 text-center">
                ...
                <p class="mt-1 text-sm text-gray-500">Account and service updates will appear here.</p>
            </div>
        @endforelse
    </div>
