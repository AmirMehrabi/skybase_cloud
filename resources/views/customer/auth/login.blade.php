...
<div class="rounded-2xl border border-slate-900/10 bg-white p-6 shadow-xl shadow-slate-900/5">
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-slate-500">
            {{ config('app.cloud.enabled') ? 'Sign in with your ISP tenant code and account email.' : 'Sign in with your account email.' }}
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('customer.login.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <input id="email" name="email" type="email" value="{{ old('email') }}" required @if (! config('app.cloud.enabled')) autofocus @endif class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-[#0d2f35] focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
            @error('email')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
