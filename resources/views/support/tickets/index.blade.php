...
<div class="space-y-6">
    ...
    <form method="GET" class="grid gap-3 rounded-xl border border-slate-900/10 bg-white p-4 shadow-sm md:grid-cols-6">
        <input name="search" value="{{ request('search') }}" placeholder="Search ticket or customer" class="rounded-lg border border-slate-300 px-3 py-2 text-sm md:col-span-2">
            ...
            <tbody class="divide-y divide-slate-900/10">
                @forelse($tickets as $ticket)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3">{{ $ticket->team?->name }}</td>
                        <td class="px-4 py-3">{{ $ticket->assignedUser?->name ?? 'Queue' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-slate-500">{{ $ticket->last_activity_at?->diffForHumans() }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-12 text-center text-slate-500">No tickets match the current filters.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
