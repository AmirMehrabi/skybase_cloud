    ...
    <form method="POST" action="{{ route('vpn-users.update', $vpnUser) }}">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                @if(auth()->user()?->isOwner())<x-ui.input.select label="User Group" name="user_group_id" :options="$userGroups" :value="old('user_group_id', $vpnUser->user_group_id)" placeholder="Ungrouped records only" :error="$errors->first('user_group_id')" />@endif
