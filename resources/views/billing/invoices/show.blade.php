    ...
    <div class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <button type="button" x-show="invoice.status !== 'void' && Number(invoice.balance_due) > 0" @click="openPaymentModal = true" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" x-transition class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50" style="display: none;">
                        ...
                        <hr class="my-1">
                        <button type="button" x-show="invoice.status !== 'void'" @click="cancelInvoice(); open = false" class="block w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50">Cancel Invoice</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-show="openPaymentModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style="display: none;">
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4" @click.away="openPaymentModal = false">
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
                <button type="button" @click="openPaymentModal = false" class="px-4 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50">
                    Cancel
                </button>
                <button type="button" @click="recordPayment()" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
                    Record Payment
                </button>
            </div>
        </div>
