function billingDashboard() {
    ...
    const chartMax = Math.max(
        ...revenueChart.flatMap((month) => [month.revenue, month.collected]),
    );
    ...
    return {
        ...
        revenueChart: revenueChart.map((month) => ({
            ...month,
            revenueHeight: chartMax > 0 ? Math.max((month.revenue / chartMax) * 100, 8) : 8,
            collectedHeight: chartMax > 0 ? Math.max((month.collected / chartMax) * 100, 8) : 8,
        })),
        recentInvoices: [
